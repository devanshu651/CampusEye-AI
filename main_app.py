...
def mark_attendance(name, dept):
    ...
    cursor.execute(
        "SELECT * FROM attendance WHERE name = ? AND timestamp > ?",
        (name, five_mins_ago)
    )

    if cursor.fetchone() is None:
        cursor.execute(
            "INSERT INTO attendance (name, department, timestamp) VALUES (?, ?, ?)",
...
def show_logs():
    ...
    cursor.execute(
        "SELECT name, department, timestamp FROM attendance "
